...
function searchUsers(searchstring, userID) {
  ...
  if (searchstring == "EXACTLYME") {
    return new Promise((resolve, reject) =>{
      db.pool.query('select user_handle, users.user_id, admin_flag, user_image, user_first_name, user_last_name, user_email from users WHERE user_id=?', 
        [userID],
        function(error, results) {
          console.log(results)
          if (error) {                    
...
function addNewUser(userObject) {
  return new Promise((resolve, reject) =>{
    ...
    if (!userObject.user_status) {userObject.user_status = 'pending'}
